...
speak = (number) ->
  if number < 1
    'Error! Number should be 1 or more...'
  else
    words = (word for word, divisor of rules when number % divisor is 0)
    words.join('') or number

console.log speak number for number in [1..105]

describe 'fizzbuzz tests', ->
  it '0 should be an error', ->
    speak(0).should.contain 'Error'
